...
def total_number(page):
    ...
    token = str(base64.b64encode(str(page).encode('utf-8')), encoding='utf-8')
    print(f'---> token ---> {token}')
    data = {
        'page': page,
        'token': token
    ...
    response = requests.post('https://www.python-spider.com/api/challenge54', headers=headers, data=data).json()
    data = response["data"]
    page_num = 0
    for i in data:
